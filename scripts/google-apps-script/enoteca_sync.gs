...
function doPost(e) {
  ...
  try {
    ...
    const qsSecret = e && e.parameter && e.parameter.secret ? String(e.parameter.secret) : '';
    ...
    const bodySecret = safeString_(body && body.secret);
    const receivedSecret = qsSecret || bodySecret;

    if (receivedSecret !== CFG.WEBHOOK_SECRET) {
      return jsonResponse_({ ok: false, status: 401, error: 'Unauthorized' });
    }

    const tableKey = resolveTableKey_(body && body.table);
    if (!tableKey) {
    ...
    syncTableFromSupabaseToSheet_(tableKey, { silentToast: true });
